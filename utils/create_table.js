...
const createTable = async () => {
  try {
    ...
    await pool.execute(
      'CREATE TABLE `user`(`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `provider` VARCHAR(10), `username` VARCHAR(20), `email` VARCHAR(45), `password` CHAR(60), `gender` CHAR(1), `intro` VARCHAR(500), `county` CHAR(3), `my_level` CHAR(1), `my_level_description` VARCHAR(500), `fans` INT UNSIGNED DEFAULT 0, `follow` INT UNSIGNED DEFAULT 0, `position_1` CHAR(1), `position_2` CHAR(1), `confirm_status` CHAR(1), `confirm_code` CHAR(60), PRIMARY KEY (`id`))'
    );
    await pool.execute(
      'CREATE TABLE `group`(`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `creator_id` INT UNSIGNED, `title` VARCHAR(20), `date` DATETIME, `time_duration` SMALLINT UNSIGNED, `net` CHAR(1), `place` VARCHAR(10), `place_description` VARCHAR(20), `court` CHAR(1), `is_charge` BOOLEAN, `money` SMALLINT UNSIGNED, `level` CHAR(1), `level_description` VARCHAR(255), `people_have` TINYINT UNSIGNED, `people_need` TINYINT UNSIGNED, `people_left` TINYINT UNSIGNED, `group_description` VARCHAR(255), `is_build` BOOLEAN, PRIMARY KEY (`id`), FOREIGN KEY (`creator_id`) REFERENCES `user` (`id`) ON DELETE CASCADE)'
    );
    await pool.execute(
    ...
    await pool.execute(
      'CREATE TABLE `comment`(`user_id` INT UNSIGNED, `commenter_id` INT UNSIGNED, `group_id` INT UNSIGNED, `score` TINYINT UNSIGNED, `content` VARCHAR(255), `date` DATETIME, PRIMARY KEY (`commenter_id`, `group_id`), FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE, FOREIGN KEY (`commenter_id`) REFERENCES `user` (`id`) ON DELETE CASCADE, FOREIGN KEY (`group_id`) REFERENCES `group` (`id`) ON DELETE CASCADE)'
    );
    await pool.execute(
      'CREATE TABLE `msg_board`(`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `user_id` INT UNSIGNED, `group_id` INT UNSIGNED, `content` VARCHAR(255), `time` DATETIME, PRIMARY KEY (`id`), FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE, FOREIGN KEY (`group_id`) REFERENCES `group` (`id`) ON DELETE CASCADE)'
    );
    await pool.execute(
